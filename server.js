...
async function start() {
  ...
  app.get('/api/admin/accounts', requireRole('admin'), async (req, res) => {
    const rows = await db.all(`SELECT id, login, role, full_name, group_name, department, position, developer_access, account_enabled, CASE WHEN password_vault IS NOT NULL THEN 1 ELSE 0 END AS password_available FROM users ORDER BY role, full_name`);
    res.json(rows);
  });
  ...
  app.post('/api/admin/accounts', requireRole('admin'), async (req, res) => {
    let { login, password, role, full_name, department, position, account_enabled = 1, developer_access = 0 } = req.body;
    ...
    try {
      ...
      await db.run(`INSERT INTO users (login, password, password_vault, role, full_name, department, position, account_enabled, developer_access) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [login.trim(), hashPassword(accountPassword), encryptPassword(accountPassword, passwordVaultKey), role, full_name.trim(), role === 'student' ? null : department?.trim() || null, role === 'student' ? null : position?.trim() || null, account_enabled ? 1 : 0, hasDeveloperAccess]);
  ...
  app.put('/api/admin/accounts/:id', requireRole('admin'), async (req, res) => {
    const { login, password, role, full_name, department, position, account_enabled, developer_access = 0 } = req.body;
    const roles = ['student', 'teacher', 'worker', 'supervisor'];
    ...
    if (password && !/^[A-Za-z0-9]{8,9}$/.test(password)) return res.status(400).json({ success: false, message: 'Пароль должен содержать 8–9 латинских букв или цифр' });
    try {
      const fields = ['login = ?', 'role = ?', 'full_name = ?', 'department = ?', 'position = ?', 'account_enabled = ?', 'developer_access = ?'];
      const params = [login.trim(), role, full_name.trim(), role === 'student' ? null : department?.trim() || null, role === 'student' ? null : position?.trim() || null, account_enabled ? 1 : 0, ['teacher', 'worker'].includes(role) && (developer_access === true || developer_access === 1) ? 1 : 0];
      if (password) {
